...
#define LFSR_MASK 0xB400E0u // 24-bit LFSR mask // 다항식(x^24 + x^22 + x^21 + x^19 + 1)
#define PAGE_SIZE 4096 
#define FIXED_SEED 0x123456 // 24-bit fixed seed

uint32_t generate_page_seed(uint32_t fixed_seed, size_t page_number) {
    return (fixed_seed ^ (page_number * 0x9E3779B1) ^ 0xABCDEFFF) & 0xFFFFFF;
}
...
void lfsr_process_page(const char* input, char* output, uint32_t fixed_seed, size_t page_number, size_t page_size_bits) {
    uint32_t page_seed = generate_page_seed(fixed_seed, page_number);
    uint32_t lfsr = page_seed;
